...
const uploadVerificationFile = (endpoint, file, timeoutMs) => {
  if (!file) {
    return Promise.reject(new Error("Select a verification file before uploading."));
  }

  if (!Number.isFinite(file.size) || file.size <= 0) {
    return Promise.reject(
      new Error("The selected verification file is empty. Please choose the file again.")
    );
  }
...
export const uploadAPI = {
  ...
  uploadAadhaarFront: (file) =>
    uploadVerificationFile(
      "/upload/verification/aadhaar-front",
      file,
      180000
    ),
  ...
  uploadAadhaarBack: (file) =>
    uploadVerificationFile(
      "/upload/verification/aadhaar-back",
      file,
      180000
    ),
  ...
  uploadFarmPhoto: (file) =>
    uploadVerificationFile(
      "/upload/verification/farm-photo",
      file,
      180000
    ),
  ...
  uploadFarmingVideo: (file) =>
    uploadVerificationFile(
      "/upload/verification/farming-video",
      file,
      600000
    ),
};
...
export const adminAPI = {
  ...
  deleteProduct: (productId) =>
    apiRequest(`/admin/products/${productId}`, { method: "DELETE" }),
